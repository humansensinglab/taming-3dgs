...
class GaussianModel:
    ...
    def restore(self, model_args, training_args):
        (self.active_sh_degree, self._xyz, self._features_dc, self._features_rest, self._scaling, self._rotation,
         self._opacity, self.max_radii2D, xyz_gradient_accum, denom, opt_dict, shopt_dict,
    ...
    def create_from_pcd(self, pcd: BasicPointCloud, spatial_lr_scale: float):
        ...
        opacities = self.inverse_opacity_activation(0.1 * torch.ones(
    ...
    def load_ply(self, path):
        ...
        self._features_dc = nn.Parameter(
            torch.tensor(features_dc, dtype=torch.float, device="cuda").transpose(1,
                                                                                  2).contiguous().requires_grad_(True))
        self._features_rest = nn.Parameter(
            torch.tensor(features_extra, dtype=torch.float,
                         device="cuda").transpose(1, 2).contiguous().requires_grad_(True))
    ...
    def densify_and_clone_taming(self, grads, budget, filter):
        grads[~filter] = 0
        n_init_points = self.get_xyz.shape[0]
